...
namespace Antmicro.Renode.Peripherals.GPIOPort
{
    public class NXPGPIOPort : BaseGPIOPort, IBusPeripheral
    {
        ...
        // hrkim 
        public uint ReadPin(int pinNumber)
        {
            if(pinNumber < 0 || pinNumber >= NumberOfConnections)
            {
                throw new ArgumentOutOfRangeException(nameof(pinNumber), $"Pin number must be between 0 and {NumberOfConnections - 1}");
            }

            lock(locker)
            {
                // this.Log(LogLevel.Info, "Read Pin : {0}", pinNumber);
                return Connections[pinNumber].IsSet ? 1u : 0u;
            }
        }
        // hrkim
        public void ResetPin(int pinNumber)
        {
            if(pinNumber < 0 || pinNumber >= NumberOfConnections)
            {
                throw new ArgumentOutOfRangeException(nameof(pinNumber), $"Pin number must be between 0 and {NumberOfConnections - 1}");
