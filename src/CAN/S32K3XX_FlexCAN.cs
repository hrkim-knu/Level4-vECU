    ...
    public partial class S32K3XX_FlexCAN : BasicDoubleWordPeripheral, IBytePeripheral, ICAN, IKnownSize
    {
        ...
        public override void WriteDoubleWord(long offset, uint value)
        {
            if(messageBufferRange.Contains(offset))
            {
                ...
                // hrkim : start
                this.Log(LogLevel.Noisy, "hrkim debug : WriteDoubleWord mboffset : {0}", mbOffset);
                this.Log(LogLevel.Noisy, "hrkim debug : WriteDoubleWord value : {0}", value);
        ...
        private bool TryTransmitFromMessageBuffer(ulong offset)
        {
            ...
            {
                this.Log(LogLevel.Noisy, "hrkim debug : messageBufferCode in TryTransmitFromMessageBuffer : {0}", (TxMessageBufferCode)messageBuffer.messageBufferCode);
                return false;
            }
        ...
        private void RunArbitrationProcess()
        {
            ...
            {
                this.Log(LogLevel.Debug, "hrkim debug : RunArbitrationProcess2-entry.Offset {0}", entry.Offset);
